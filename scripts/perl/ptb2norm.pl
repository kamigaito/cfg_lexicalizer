...
sub main {
    ...
    # Conversion with head information.
        ...
        while(1){
            my @const = &string_to_internal($const_fp);
            my @dep = &dep_to_internal($dep_fp);
            my @aligned = &head_search(\@const, \@dep);
            my @aligned_norm = &pos_norm(\@aligned, \%skip_list);
            &output_to_files_lex($out_const_fp,$out_align_fp,\@aligned_norm);
        }
        close($const_fp);
        close($dep_fp);
        close($out_const_fp);
        ...
        print STDERR "Malformed options!\n";
        exit(1);
    }
}
...
sub dep_to_internal {
    ...
    while(my $line = <$dep_fp>){
        ...
        if(scalar(@col) == 3){
            push(@dep, $col[2] - 1);
        # conll-x and conll-u (Stanford dependency)
        }elsif(scalar(@col) >= 10){
            push(@dep, ($col[6] - 1));
        }else{
